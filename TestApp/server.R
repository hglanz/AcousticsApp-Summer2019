...
output$maxtimelimit <- renderUI({
  ...
  if (!is.null(inFile)) {
    ...
    numericInput("maxtime", label = "Maximum Time (s):",
                 value = length(wav@left)/wav@samp.rate, min = 0, max = length(wav@left)/wav@samp.rate)
  } else {
    wav <- switch(filevalues$file2,
                  "2" = sine1,
                  "3" = sine2,
...
output$maxfreqlimit <- renderUI({
  ...
  if (!is.null(inFile)) {
    ...
    if (length(maxfreq_pos) == 0) {
      ...
      numericInput("maxfreq", label = "Maximum Frequency (kHz):",
                   value = min(rev(sp$freq)[maxfreq_pos]+.5, max(sp$freq)))
    }
  } else {
    wav <- switch(filevalues$file2,
                  "2" = sine1,
    ...
    if (times %% 2 == 1) {
        return("<p><b>Zero Padding Help</b></p>
               ...
               <p>One way to manage this and to increase the frequency resolution
                    without loosing time resolution is to extend a signal (artificially)
